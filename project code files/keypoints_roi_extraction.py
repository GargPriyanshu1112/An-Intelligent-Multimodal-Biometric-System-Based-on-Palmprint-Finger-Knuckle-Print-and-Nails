    ...
    if results.multi_hand_landmarks:
        for _, landmarks in enumerate(results.multi_hand_landmarks):
            mp_drawing.draw_landmarks(lmarks_img, landmarks, None,
                                      mp_drawing.DrawingSpec(color=(0,0,0), thickness=20, circle_radius=4))
...
for i in range(2, 21):
    ...
    image_with_bb = cv2.rectangle(image_copy, 
...
for idx, roi in enumerate(keypoint_rois):
