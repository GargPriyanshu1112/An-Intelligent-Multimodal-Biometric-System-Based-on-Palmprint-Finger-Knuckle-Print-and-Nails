...
# Set up paths
IMAGES_DIRPATH = "E:/college_project/11K_Hands2/"
METADATA_FILEPATH = "E:/college_project/HandInfo.csv"
DATASET_DIRPATH = "E:/college_project/college_dataset"
...
def save_files(ID, idx, files, dst_dirpath):
    for fname in files:
        os.replace(os.path.join(IMAGES_DIRPATH, fname),
                   os.path.join(dst_dirpath, f"id-{ID}--{idx}.jpg"))
...
def get_augmented_images_and_save_them(ID, num_reqd, filename, dst_dirpath):
    ...
    while(num_reqd > 0):
        ...
        aug_img_name = f"id-{ID}--{idx}_aug.jpg"
...
def palmar_train_test_split(ID, n_train):
    os.mkdir(os.path.join(PALMAR_TRAIN, str(ID)))
    os.mkdir(os.path.join(PALMAR_TEST, str(ID)))
    ...
    for fname in train_fnames:
        os.replace(os.path.join(EXTRAS_FOLDER, fname),
                   os.path.join(os.path.join(PALMAR_TRAIN, str(ID), fname)))
        
    # Save remaining images as test images
    test_fnames = os.listdir(EXTRAS_FOLDER)
    for fname in test_fnames:
        os.replace(os.path.join(EXTRAS_FOLDER, fname),
                   os.path.join(os.path.join(PALMAR_TEST, str(ID), fname)))
        

def dorsal_train_test_split(ID, n_train):
    os.mkdir(os.path.join(DORSAL_TRAIN, str(ID)))
    os.mkdir(os.path.join(DORSAL_TEST, str(ID)))
    ...
    for fname in train_fnames:
        os.replace(os.path.join(EXTRAS_FOLDER, fname),
                   os.path.join(os.path.join(DORSAL_TRAIN, str(ID), fname)))
        
    # Save remaining images as test images
    test_fnames = os.listdir(EXTRAS_FOLDER)
    for fname in test_fnames:
        os.replace(os.path.join(EXTRAS_FOLDER, fname),
                   os.path.join(os.path.join(DORSAL_TEST, str(ID), fname)))
...
for ID in unique_ids[2:]:
    ...
    if (num_palmar_imgs and num_dorsal_imgs):
        ...
        if num_palmar_imgs < REQD_PALMAR_IMGS+1:
            ...
            save_files(ID=ID, idx=idx, files=palmar_fnames_list, dst_dirpath=EXTRAS_FOLDER)
        
        else:
            # Save required number of imagefiles
            save_files(ID=ID, idx=1, files=palmar_fnames_list, dst_dirpath=EXTRAS_FOLDER)

        palmar_train_test_split(ID, n_train=8)
        ...
        if num_dorsal_imgs < REQD_DORSAL_IMGS+1:
            ...
            # Save original imagefiles
            save_files(ID=ID, idx=idx, files=dorsal_fnames_list, dst_dirpath=EXTRAS_FOLDER)
            
        else:
            # Save required original imagefiles
            save_files(ID=ID, idx=1, files=dorsal_fnames_list, dst_dirpath=EXTRAS_FOLDER)
        
        dorsal_train_test_split(ID, n_train=8)
